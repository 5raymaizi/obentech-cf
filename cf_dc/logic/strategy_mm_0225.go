...
func (sg *Strategy) openSwap1AndSwap2(ctx context.Context, swapC1, swapC2 exchange.SwapClient,
	...
	var (
		...
		inTimeStopLoss    bool      // 是否在时间内止损
		timeStopLostStart time.Time // 时间止损开始时间
	)
	cfg := sg.load.Load()
	one := decimal.NewFromFloat(1)
	enableTsl := cfg.MMEnableTimeStopLoss
	...
	for {
		...
		case <-maxDurationTimer.C:
			...
			go message.SendP3Important(ctx, message.NewCommonMsg(fmt.Sprintf("%s MM开仓超时", sg.swapS1), fmt.Sprintf("操作ID: %s, 运行超过20分钟强制退出", op.GetOpID())))
			takerHedge = true
			goto checkAndHedge
		case <-ctx.Done():
		...
		case <-sg.bookTickerEvent.Done(): // bookTicker变动, 同步检测CancelClose
			...
			hasFilled := swap1Order.Filled.GreaterThan(decimal.Zero) || swap2Order.Filled.GreaterThan(decimal.Zero)

			if side == ccexgo.OrderSideBuy { // 做空价差
				_srOpen := swap2Ask1Price.Div(swap1Bid1Price).Sub(one)
				...
				if !enableTsl && takerSr.LessThan(decimal.NewFromFloat(cfg.MmOpenTakerHedgeThreshold)) {
					takerHedge = true
				...
				} else if !enableTsl && nowSr.LessThan(decimal.NewFromFloat(cfg.MmOpenMakerHedgeThreshold)) && !makerHedge {
					if order1 != nil && order1.Status < ccexgo.OrderStatusDone {
						swap1Cancel = true
					}
					if order2 != nil && order2.Status < ccexgo.OrderStatusDone {
				...
				} else if enableTsl && !makerHedge && hasFilled {
					// 进行一边固定
					srOpen = _srOpen.InexactFloat64()
					...
					if swap1Order.Filled.GreaterThan(swap2Order.Filled) && !swap2End {
						...
						swap1End = true
						// newSwap2Price := swap2Bid1Price.Add(decimal.NewFromInt(int64(cfg.MmSwap2OpenMakerHedgeTickNum)).Mul(pricePrecision2))
						newSwap2Price := decimal.Max(swap2Ask1Price, swap1Order.AvgPrice.Mul(one.Add(_srOpen)).Round(sg.priceExp2))
						if order2 != nil && newSwap2Price.Equal(order2.Price) && order2.Status != ccexgo.OrderStatusCancel { // 和当前挂单价一致，则不撤单
							swap2Cancel = false
							swap2NeedReplace = false
							timeStopLostStart = time.Now()
						}
						makerHedge = true
					} else if swap1Order.Filled.LessThan(swap2Order.Filled) && !swap1End {
						swap1NeedReplace = true
						swap2End = true
						// newSwap1Price := swap1Ask1Price.Sub(decimal.NewFromInt(int64(cfg.MmSwap1OpenMakerHedgeTickNum)).Mul(pricePrecision1))
						newSwap1Price := decimal.Min(swap1Bid1Price, swap2Order.AvgPrice.Div(one.Add(_srOpen).Round(sg.priceExp1)))
						if order1 != nil && newSwap1Price.Equal(order1.Price) && order1.Status != ccexgo.OrderStatusCancel { // 和当前挂单价一致，则不撤单
							swap1Cancel = false
							swap1NeedReplace = false
							timeStopLostStart = time.Now()
					...
					inTimeStopLoss = true
					level.Info(logger).Log("message", "open use time stop loss", "nowSr", nowSr, "swap1Ask1Price", swap1Ask1Price, "swap1Bid1Price", swap1Bid1Price, "swap2Ask1Price", swap2Ask1Price, "swap2Bid1Price", swap2Bid1Price,
						"swap1Cancel", swap1Cancel, "swap2Cancel", swap2Cancel, "swap1NeedReplace", swap1NeedReplace, "swap2NeedReplace", swap2NeedReplace, "makerHedge", makerHedge, "swap1End", swap1End, "swap2End", swap2End)
				} else {
					// 如果在时间止损内, 不检查跟随盘口
					if inTimeStopLoss && (timeStopLostStart.IsZero() || (!timeStopLostStart.IsZero() && time.Since(timeStopLostStart).Seconds() < float64(cfg.MMTimeStopLossWait))) {
						// 如果需要重挂，跳过 continue，让代码继续执行到重挂逻辑
						if !swap1NeedReplace && !swap2NeedReplace {
							continue
						}
						// 需要重挂时，保持 inTimeStopLoss = true，跳过盘口跟随检查
					} else {
						inTimeStopLoss = false

						// 记录进入盘口跟随检查前的状态，用于判断是否有变化
						prevSwap1Cancel := swap1Cancel
						prevSwap2Cancel := swap2Cancel
						prevSwap1NeedReplace := swap1NeedReplace
						prevSwap2NeedReplace := swap2NeedReplace
						...
						// 只有在关键参数发生变化时才打印日志
						if swap1Cancel != prevSwap1Cancel || swap2Cancel != prevSwap2Cancel ||
							swap1NeedReplace != prevSwap1NeedReplace || swap2NeedReplace != prevSwap2NeedReplace {
				...
				_srOpen := swap2Bid1Price.Div(swap1Ask1Price).Sub(one)
				...
				if !enableTsl && takerSr.GreaterThan(decimal.NewFromFloat(cfg.MmCloseTakerHedgeThreshold)) {
					takerHedge = true
					level.Info(logger).Log("message", "open nowSr > closeTakerHedgeThreshold ", "nowSr", nowSr, "takerSr", takerSr, "swap1Ask1Price", swap1Ask1Price, "swap1Bid1Price", swap1Bid1Price, "swap2Ask1Price", swap2Ask1Price, "swap2Bid1Price", swap2Bid1Price)
				} else if !enableTsl && nowSr.GreaterThan(decimal.NewFromFloat(cfg.MmCloseMakerHedgeThreshold)) && !makerHedge {
					if order1 != nil && order1.Status < ccexgo.OrderStatusDone {
						swap1Cancel = true
					}
					if order2 != nil && order2.Status < ccexgo.OrderStatusDone {
					...
					level.Info(logger).Log("message", "open nowSr > closeMakerHedgeThreshold ", "nowSr", nowSr, "swap1Ask1Price", swap1Ask1Price, "swap1Bid1Price", swap1Bid1Price, "swap2Ask1Price", swap2Ask1Price, "swap2Bid1Price", swap2Bid1Price,
						"swap1Cancel", swap1Cancel, "swap2Cancel", swap2Cancel, "swap1NeedReplace", swap1NeedReplace, "swap2NeedReplace", swap2NeedReplace, "makerHedge", makerHedge, "swap1End", swap1End, "swap2End", swap2End)
					...
					level.Info(logger).Log("message", "open nowSr > cancelCloseThreshold ", "nowSr", nowSr, "swap1Ask1Price", swap1Ask1Price, "swap1Bid1Price", swap1Bid1Price, "swap2Ask1Price", swap2Ask1Price, "swap2Bid1Price", swap2Bid1Price,
						"swap1Cancel", swap1Cancel, "swap2Cancel", swap2Cancel, "swap1NeedReplace", swap1NeedReplace, "swap2NeedReplace", swap2NeedReplace, "finishedWork", finishedWork, "swap1End", swap1End, "swap2End", swap2End, "cancelFilled", cancelFilled)
				} else if enableTsl && !makerHedge && hasFilled {
					// 进行一边固定
					srOpen = _srOpen.InexactFloat64()
					...
					if swap1Order.Filled.GreaterThan(swap2Order.Filled) && !swap2End {
						...
						swap1End = true
						// newSwap2Price := swap2Ask1Price.Sub(decimal.NewFromInt(int64(cfg.MmSwap2CloseMakerHedgeTickNum)).Mul(pricePrecision2))
						newSwap2Price := decimal.Min(swap2Bid1Price, swap1Order.AvgPrice.Mul(one.Add(decimal.NewFromFloat(srOpen)).Round(int32(sg.priceExp2))))
						if order2 != nil && newSwap2Price.Equal(order2.Price) && order2.Status != ccexgo.OrderStatusCancel { // 和当前挂单价一致，则不撤单
							swap2Cancel = false
							swap2NeedReplace = false
							timeStopLostStart = time.Now()
						...
						swap2End = true
						// newSwap1Price := swap1Bid1Price.Add(decimal.NewFromInt(int64(cfg.MmSwap1CloseMakerHedgeTickNum)).Mul(pricePrecision1))
						newSwap1Price := decimal.Max(swap1Ask1Price, swap2Order.AvgPrice.Div(one.Add(decimal.NewFromFloat(srOpen)).Round(int32(sg.priceExp1))))
						if order1 != nil && newSwap1Price.Equal(order1.Price) && order1.Status != ccexgo.OrderStatusCancel { // 和当前挂单价一致，则不撤单
							swap1Cancel = false
							swap1NeedReplace = false
							timeStopLostStart = time.Now()
			...
			if (order1 == nil || order1.Status >= ccexgo.OrderStatusDone) && swap1NeedReplace {
				...
				_, o, errMsg1, err := sg.PosList.OpenSwapMaker(ctx, swapC1, srOpen, openAmount, float64(tickNum), sg.swap1OrderBook, op, side, 0, false, inTimeStopLoss, swap2Order.AvgPrice)
				if err != nil {
				...
				if inTimeStopLoss && timeStopLostStart.IsZero() {
					timeStopLostStart = time.Now()
				}
				// 考虑下单即成交的情况
				if !o.Filled.IsZero() {
		...
		replace2:
			if (order2 == nil || order2.Status >= ccexgo.OrderStatusDone) && swap2NeedReplace {
				...
				_, o, errMsg1, err := sg.PosList.OpenSwapMaker(ctx, swapC2, srOpen, openAmount, float64(tickNum), sg.swap2OrderBook, op, reverseSide(side), 0, false, inTimeStopLoss, swap1Order.AvgPrice)
				if err != nil {
					errType := swapC2.HandError(err)
...
checkAndHedge:
	...
	if takerHedge || finishedWork || makerReplaceFailed {
		...
		if needTakerHedgeAmount.IsPositive() {
			...
			needTakerHedgeAmount = needTakerHedgeAmount.Round(int32(sg.amountExp2))
			...
			swapClient = swapC1
			placeSide = side
			needTakerHedgeAmount = needTakerHedgeAmount.Abs().Round(int32(sg.amountExp1))
...
func (sg *Strategy) closeSwap1AndSwap2(ctx context.Context, swapC1, swapC2 exchange.SwapClient,
	...
	for {
		...
		case <-maxDurationTimer.C:
			...
			go message.SendP3Important(ctx, message.NewCommonMsg(fmt.Sprintf("%s MM平仓超时", sg.swapS1), fmt.Sprintf("操作ID: %s, 运行超过20分钟强制退出", op.GetOpID())))
			takerHedge = true
			goto checkAndHedge
		case <-ctx.Done():
			// 收到退出信号，立即执行撤单和清理逻辑（复用原 checkExitNowTimer 的处理逻辑）
		...
		case <-sg.bookTickerEvent.Done():
			...
			if side == ccexgo.OrderSideCloseShort { // 平空， buy
				_srClose := swap2Ask1Price.Div(swap1Bid1Price).Sub(one)
				...
				if !enableTsl && takerSr.LessThan(decimal.NewFromFloat(cfg.MmOpenTakerHedgeThreshold)) {
					...
					srClose = _srClose.InexactFloat64()
					...
					if swap1Order.Filled.GreaterThan(swap2Order.Filled) && !swap2End {
						...
						swap1End = true
						// newSwap2Price := swap2Bid1Price.Add(decimal.NewFromInt(int64(cfg.MmSwap2OpenMakerHedgeTickNum)).Mul(pricePrecision2))
						newSwap2Price := decimal.Max(swap2Ask1Price, swap1Order.AvgPrice.Mul(one.Add(_srClose)).Round(sg.priceExp2))
						if order2 != nil && newSwap2Price.Equal(order2.Price) && order2.Status != ccexgo.OrderStatusCancel { // 和当前挂单价一致，则不撤单
							swap2Cancel = false
							swap2NeedReplace = false
							timeStopLostStart = time.Now()
						...
						swap2End = true
						// newSwap1Price := swap1Ask1Price.Sub(decimal.NewFromInt(int64(swap1OpenTick)).Mul(pricePrecision1))
						newSwap1Price := decimal.Min(swap1Bid1Price, swap2Order.AvgPrice.Div(one.Add(_srClose).Round(sg.priceExp1)))
						if order1 != nil && newSwap1Price.Equal(order1.Price) && order1.Status != ccexgo.OrderStatusCancel { // 和当前挂单价一致，则不撤单
							swap1Cancel = false
							swap1NeedReplace = false
							timeStopLostStart = time.Now()
					...
					inTimeStopLoss = true
					level.Info(logger).Log("message", "close use time stop loss", "nowSr", nowSr, "swap1Ask1Price", swap1Ask1Price, "swap1Bid1Price", swap1Bid1Price, "swap2Ask1Price", swap2Ask1Price, "swap2Bid1Price", swap2Bid1Price,
						"swap1Cancel", swap1Cancel, "swap2Cancel", swap2Cancel, "swap1NeedReplace", swap1NeedReplace, "swap2NeedReplace", swap2NeedReplace, "makerHedge", makerHedge, "swap1End", swap1End, "swap2End", swap2End)
				} else {
					// 如果在时间止损内, 不检查跟随盘口
					if inTimeStopLoss && (timeStopLostStart.IsZero() || (!timeStopLostStart.IsZero() && time.Since(timeStopLostStart).Seconds() < float64(cfg.MMTimeStopLossWait))) {
				...
				_srClose := swap2Bid1Price.Div(swap1Ask1Price).Sub(one)
				...
				if !enableTsl && takerSr.GreaterThan(decimal.NewFromFloat(cfg.MmCloseTakerHedgeThreshold)) {
					...
					if swap1Order.Filled.GreaterThan(swap2Order.Filled) && !swap1End {
						...
						swap1End = true
						// newSwap2Price := swap2Ask1Price.Sub(decimal.NewFromInt(int64(cfg.MmSwap2CloseMakerHedgeTickNum)).Mul(pricePrecision2))
						newSwap2Price := decimal.Min(swap2Bid1Price, swap1Order.AvgPrice.Mul(one.Add(decimal.NewFromFloat(srClose)).Round(int32(sg.priceExp2))))
						if order2 != nil && newSwap2Price.Equal(order2.Price) && order2.Status != ccexgo.OrderStatusCancel { // 和当前挂单价一致，则不撤单
							swap2Cancel = false
							swap2NeedReplace = false
							timeStopLostStart = time.Now()
						...
						swap2End = true
						// newSwap1Price := swap1Bid1Price.Add(decimal.NewFromInt(int64(cfg.MmSwap1CloseMakerHedgeTickNum)).Mul(pricePrecision1))
						newSwap1Price := decimal.Max(swap1Ask1Price, swap2Order.AvgPrice.Div(one.Add(decimal.NewFromFloat(srClose)).Round(int32(sg.priceExp1))))
						if order1 != nil && newSwap1Price.Equal(order1.Price) && order1.Status != ccexgo.OrderStatusCancel { // 和当前挂单价一致，则不撤单
							swap1Cancel = false
							swap1NeedReplace = false
							timeStopLostStart = time.Now()
			...
			if (order1 == nil || order1.Status >= ccexgo.OrderStatusDone) && swap1NeedReplace {
				...
				_, o, errMsg1, err := sg.PosList.CloseSwapMaker(ctx, swapC1, srClose, closeAmount, float64(tickNum), sg.swap1OrderBook, op, side, 0, false, false, inTimeStopLoss, swap2Order.AvgPrice)
				if err != nil {
					errType := swapC1.HandError(err)
		...
		replace2:
			if (order2 == nil || order2.Status >= ccexgo.OrderStatusDone) && swap2NeedReplace {
				...
				_, o, errMsg1, err := sg.PosList.CloseSwapMaker(ctx, swapC2, srClose, closeAmount, float64(tickNum), sg.swap2OrderBook, op, reverseSide(side), 0, false, false, inTimeStopLoss, swap1Order.AvgPrice)
				if err != nil {
					errType := swapC2.HandError(err)
